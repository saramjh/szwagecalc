...
const HamburgerMenu = ({ session, onLogout, username, onOpenJobManagementModal, deferredPrompt, onInstallPWA }) => {
	...
	const handleFeedbackClick = () => {
		const subject = encodeURIComponent("[시급이요] 피드백")
		const body = encodeURIComponent(`안녕하세요, 시급이요 앱에 대한 피드백입니다.\n\n사용자: ${username || session?.user?.email || "알 수 없음"}\n\n[여기에 피드백 내용을 작성해주세요]\n\n`)
		window.location.href = `mailto:devtestudinidae@gmail.com?subject=${subject}&body=${body}`
	...
	return (
		<div className="relative" ref={menuRef}>
			...
			{isOpen && (
				<div className="absolute right-0 mt-2 w-48 bg-cream-white dark:bg-charcoal-gray rounded-md shadow-lg py-1 z-layer-dropdown transition-all duration-300 ease-out transform origin-top-right scale-100">
