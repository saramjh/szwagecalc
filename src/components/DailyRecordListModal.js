    ...
    <div className={`fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 transition-opacity duration-300 ease-out ${animateModal ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
      <div className={`bg-cream-white dark:bg-charcoal-gray rounded-2xl shadow-lg p-6 w-full max-w-md transform transition-all duration-300 ease-out ${animateModal ? 'translate-y-0' : 'translate-y-10'}`}>
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-bold text-dark-navy dark:text-white">{moment(selectedDate).format('YYYY년 M월 D일 (ddd)')} 근무 기록</h2>
        ...
        <div className="space-y-4 mb-6 max-h-60 overflow-y-auto">
          {dailyRecords.length === 0 ? (
            ...
            dailyRecords.map((record) => (
              <div key={record.id} className="flex justify-between items-center py-2 border-b border-gray-100 dark:border-gray-600 last:border-b-0">
                <div>
                  <p className="text-dark-navy dark:text-white font-medium">
                    {record.jobs?.job_name && <span className="text-mint-green mr-2">[{record.jobs.job_name}]</span>}
                    {record.start_time} ~ {record.end_time}
                  </p>
                  <p className="text-sm text-medium-gray dark:text-light-gray ml-2">식대: {record.meal_allowance.toLocaleString()}원</p>
                  {record.notes && <p className="text-sm text-medium-gray dark:text-light-gray ml-2">비고: {record.notes}</p>}
                  <p className="text-sm text-coral-pink ml-2 font-semibold">일급: {record.daily_wage.toLocaleString()}원</p>
                </div>
                <div className="flex space-x-2">
                  <button
                    onClick={() => handleEditRecord(record)}
                    className="text-mint-green hover:text-mint-green-dark transition-colors duration-200"
                  >
                    편집
                  </button>
                  <button
                    onClick={() => handleDeleteRecord(record.id)}
                    className="text-coral-pink hover:text-coral-pink-dark transition-colors duration-200"
                  >
                    삭제
                  </button>
                </div>
