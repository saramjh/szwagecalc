...
시급이요(szwagecalc)는 PWA(Progressive Web App)를 지원하여 앱처럼 사용할 수 있습니다. 아래 안내에 따라 휴대폰 홈 화면에 추가해보세요.
...
`npm run deploy` 스크립트를 실행하여 GitHub Pages에 배포할 수 있습니다.
...
- **월급 보고서 성능 개선**: 월급 보고서 모달에서 발생하는 무한 네트워크 요청 버그를 수정하여 안정성과 성능을 향상시켰습니다.
- **데이터 로직 안정화**: 데이터 의존성에 따라 `useEffect` 로직을 분리하여, 데이터가 표시되는 과정에서 발생할 수 있는 경쟁 상태(race condition)를 방지하고 데이터 정합성을 확보했습니다.
- **코드 품질 향상**: ESLint 훅 의존성 관련 경고를 해결하여 코드 품질을 개선했습니다.
